...
- keep pruning dead helper state and setup duplication where live/replay still carry cheap mirrors
...
- prefer docs and contracts that describe the current single-core model directly, instead of preserving historical scaffolding

At this point the architecture is much closer to "one core with a few explicit edges" than to a real split-brain system. The remaining cleanup is mostly optional tightening, not ownership convergence.
