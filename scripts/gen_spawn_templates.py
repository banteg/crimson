...
def _render_status_table() -> str:
    _repo_import()
    from crimson.creatures.spawn import SPAWN_IDS_PORTED, SPAWN_IDS_VERIFIED, SPAWN_TEMPLATES  # noqa: PLC0415

    # Match docs/metrics.md legend: ✅ complete · 🚧 in progress · ⬜ not started
    def _ported_marker(spawn_id: int) -> str:
        return "✅" if spawn_id in SPAWN_IDS_PORTED else "⬜"

    def _verified_marker(spawn_id: int) -> str:
        if spawn_id in SPAWN_IDS_VERIFIED:
            return "✅"
        if spawn_id in SPAWN_IDS_PORTED:
            return "🚧"
        return "⬜"
    ...
    for entry in sorted(SPAWN_TEMPLATES, key=lambda t: t.spawn_id):
        creature = "" if entry.creature is None else entry.creature
        ported = _ported_marker(entry.spawn_id)
        verified = _verified_marker(entry.spawn_id)
        lines.append(f"| `0x{entry.spawn_id:x}` | `{creature}` | {ported} | {verified} |")
    lines.append("")
    return "\n".join(lines)
