...
def tick_fire_cough(ctx: PlayerPerkTickCtx) -> None:
    ...
    dir_angle = float(ctx.state.rng.rand(caller=RngOwnerStatic.PLAYER_UPDATE) & 0x1FF) * (
        math.tau / 512.0
    )
    mag = float(ctx.state.rng.rand(caller=RngOwnerStatic.PLAYER_UPDATE) & 0x1FF) * (1.0 / 512.0)
    ...
    ctx.state.perk_intervals.fire_cough = (
        float(ctx.state.rng.rand(caller=RngOwnerStatic.PLAYER_UPDATE) % 4) + 2.0
    )
