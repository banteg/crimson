...
def perk_select_random(state: GameplayState, player: PlayerState, *, game_mode: GameMode, player_count: int) -> PerkId:
    ...
    for _ in range(1000):
        perk_id = PerkId(
            state.rng.rand(caller=RngOwnerStatic.PERK_SELECT_RANDOM) % PERK_ID_MAX + 1,
        )
...
def perk_generate_choices(
    ...
    *,
    ...
    def _select_random_offer() -> PerkId:
        for _ in range(1000):
            perk_index = state.rng.rand(caller=RngOwnerStatic.PERK_SELECT_RANDOM) % PERK_ID_MAX + 1
    ...
    while choice_index < 7:
        ...
        while True:
            ...
            if (
                ...
                and (state.rng.rand(caller=RngOwnerStatic.PERK_SELECT_RANDOM) & 3) == 1
            ):
                continue
    ...
    *,
    ...
    while perk_state.pending_count > 0:
        ...
        if not visible_choices:
            break
        idx = int(state.rng.rand(caller=RngOwnerStatic.PERK_SELECT_RANDOM) % len(visible_choices))
