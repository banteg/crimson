...
def _native_shot_angle_with_jitter(
    *,
    ...
    dir_angle = float(
        f32(float(rng.rand(caller=RngOwnerStatic.PLAYER_FIRE_WEAPON) & 0x1FF) * (float(NATIVE_TAU) / 512.0)),
    )
    mag = float(f32(float(rng.rand(caller=RngOwnerStatic.PLAYER_FIRE_WEAPON) & 0x1FF) * (1.0 / 512.0)))
    ...
    match jitter_rule:
        ...
        case ModuloCenteredJitter(modulo=modulo, center=center, step=step):
            return float(shot_angle) + float(
                rng.rand(caller=RngOwnerStatic.PLAYER_FIRE_WEAPON) % int(modulo) - int(center),
            ) * float(step)
        case MaskCenteredJitter(mask=mask, center=center, step=step):
            return float(shot_angle) + float(
                (rng.rand(caller=RngOwnerStatic.PLAYER_FIRE_WEAPON) & int(mask)) - int(center),
            ) * float(step)
    ...
    match speed_rule:
        ...
        case ModuloSpeedScale(base=base, modulo=modulo, step=step):
            state.projectiles.entries[int(proj_id)].speed_scale = float(base) + float(
                state.rng.rand(caller=RngOwnerStatic.PLAYER_FIRE_WEAPON) % int(modulo),
            ) * float(step)
...
def fire_weapon(ctx: WeaponFireCtx) -> WeaponFireResult:
    ...
    if (not force_pre_swap_fire_gate) and player.weapon.reload_timer > 0.0:
        ...
        if perk_active(player, PerkId.REGRESSION_BULLETS):
            ...
            player_take_damage(
                ...
                cost,
                dt=dt,
                caller=RngOwnerStatic.PLAYER_FIRE_WEAPON,
                players=players,
    ...
    if weapon_flags & 0x1:
        shell_casing_draws = (
            state.rng.rand(caller=RngOwnerStatic.PLAYER_FIRE_WEAPON),
            state.rng.rand(caller=RngOwnerStatic.PLAYER_FIRE_WEAPON),
            state.rng.rand(caller=RngOwnerStatic.PLAYER_FIRE_WEAPON),
            state.rng.rand(caller=RngOwnerStatic.PLAYER_FIRE_WEAPON),
        )
