...
_BISECT_BLOCK_DECODER = msgspec.msgpack.Decoder(type=BisectTickBlock)
...
def _write_bisect_trace_from_iter(
    path: Path,
    *,
    meta: TraceMeta,
    ticks: Iterable[BisectTickRecord],
    chunk_ticks: int,
) -> TraceSummary:
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    ...
    with path.open("wb") as handle:
        ...
        last_tick: int | None = None
        tick_count = 0
        current_block: list[BisectTickRecord] = []

        def flush_block() -> None:
            nonlocal first_tick, last_tick, tick_count
            if not current_block:
                return
            block = BisectTickBlock(
                start_tick=int(current_block[0].tick_index),
                end_tick=int(current_block[-1].tick_index),
                ticks=list(current_block),
            )
            ...
            for row in current_block:
                ...
                last_tick = row_tick
                tick_count += 1
                for channel_name in ("golden", "candidate", "focus_tick"):
                    channel_counts[channel_name] = channel_counts.get(channel_name, 0) + 1
            current_block.clear()
...
def write_bisect_trace_iter(
    ...
    *,
    ...
    return _write_bisect_trace_from_iter(
        path,
        meta=meta,
        ticks=ticks,
        chunk_ticks=max(1, int(chunk_ticks)),
...
def write_bisect_trace(
    path: Path,
    *,
    meta: TraceMeta,
    ticks: Sequence[BisectTickRecord],
    chunk_ticks: int = 256,
) -> TraceSummary:
    sorted_ticks = sorted((tick for tick in ticks), key=lambda row: int(row.tick_index))
    return _write_bisect_trace_from_iter(
...
class BisectTraceReader:
    def __init__(self, path: Path) -> None:
        ...
        if len(self.footer.tick_blocks) <= 0:
            raise TraceError("invalid trace footer block index")
        self._block_cache: dict[int, BisectTickBlock] = {}

    def close(self) -> None:
        if not self._handle.closed:
            self._handle.close()

    def __enter__(self) -> "BisectTraceReader":
        return self

    def __exit__(self, *_args: object) -> None:
        self.close()

    def _load_block(self, entry: TickBlockIndexEntry) -> BisectTickBlock:
        cache_key = entry.file_offset
        cached = self._block_cache.get(cache_key)
        if cached is not None:
            return cached
        ...
        if kind != CHUNK_KIND_TICK:
            raise TraceError("trace index points at non-tick chunk")
        try:
            block = _BISECT_BLOCK_DECODER.decode(payload)
        except (msgspec.DecodeError, msgspec.ValidationError) as exc:
            raise TraceError("invalid bisect trace tick payload") from exc
        self._block_cache[cache_key] = block
        return block

    def tick(self, tick_index: int) -> BisectTickRecord | None:
        tick = tick_index
        for entry in self.footer.tick_blocks:
            if entry.start_tick <= tick <= entry.end_tick:
                block = self._load_block(entry)
    ...
    def all_ticks(self) -> list[BisectTickRecord]:
        return list(self.iter_ticks())
