...
class FontView:
    def __init__(self, ctx: ViewContext) -> None:
        assets_root = ctx.assets_dir
        self._missing_assets: list[str] = []
        self._small = self._load_small_font(assets_root)
        self._mono = self._load_mono_font(assets_root)
    ...
    def _load_mono_font(self, assets_root: Path) -> MonoFontData | None:
        ...
        if not atlas_path.is_file():
            ...
            return None
        texture = rl.load_texture(str(atlas_path))
        columns = max(1, texture.width // 16)
        return MonoFontData(texture=texture, columns=columns)
    ...
    def _draw_mono_text(self, text: str, x: float, y: float, scale: float, color: rl.Color) -> None:
        ...
        for value in text.encode("latin-1", errors="replace"):
            ...
            if value == 0x0D:
                continue
            col = value % font.columns
            row = value // font.columns
            src = rl.Rectangle(
                float(col * font.cell_size),
                float(row * font.cell_size),
                float(font.cell_size),
