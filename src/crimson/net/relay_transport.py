...
from .relay_protocol import RelayPacket, decode_packet, encode_packet
...
class RelayUdpTransport(msgspec.Struct):
    ...
    def send_packet(self, addr: PeerAddr, packet: RelayPacket) -> None:
        sock = self._sock
        if sock is None:
    ...
    def recv_packets(self, *, max_packets: int | None = None) -> list[tuple[PeerAddr, RelayPacket]]:
        sock = self._sock
        if sock is None:
            return []
        out: list[tuple[PeerAddr, RelayPacket]] = []
