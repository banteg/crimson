...
def player_take_damage(
    ...
    *,
    ...
    if perk_active(player, PerkId.NINJA):
        dodged = (int(state.rng.rand(caller=caller)) % 3) == 0
    elif perk_active(player, PerkId.DODGER):
        dodged = (int(state.rng.rand(caller=caller)) % 5) == 0

    health_before = float(player.health)
    if not dodged:
        if perk_active(player, PerkId.HIGHLANDER):
            if (int(state.rng.rand(caller=caller)) % 10) == 0:
                player.health = 0.0
        else:
    ...
    if not lethal_hit:
        state.sfx_queue.append(
            _PLAYER_PAIN_SFX[int(state.rng.rand(caller=caller)) % len(_PLAYER_PAIN_SFX)],
        )
        if not was_alive:
        ...
        if not perk_active(player, PerkId.FINAL_REVENGE):
            state.sfx_queue.append(_PLAYER_DEATH_SFX[int(state.rng.rand(caller=caller)) & 1])
    ...
    if not dodged:
        if not perk_active(player, PerkId.UNSTOPPABLE):
            player.heading += float((int(state.rng.rand(caller=caller)) % 100) - 50) * 0.04
        ...
        if player.health <= 20.0 and (int(state.rng.rand(caller=caller)) & 7) == 3:
