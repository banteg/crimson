...
_PLAYER_TAKE_DAMAGE_NINJA_CALLER = 0x00425F23
_PLAYER_TAKE_DAMAGE_DODGER_CALLER = 0x00425F4E
_PLAYER_TAKE_DAMAGE_HIGHLANDER_CALLER = 0x00425F79
_PLAYER_TAKE_DAMAGE_DEATH_SFX_CALLER = 0x00426124
_PLAYER_TAKE_DAMAGE_PAIN_SFX_CALLER = 0x00426152
_PLAYER_TAKE_DAMAGE_HEADING_CALLER = 0x00426197
_PLAYER_TAKE_DAMAGE_LOW_HEALTH_CALLER = 0x00426201
...
def player_take_damage(
    ...
    *,
    ...
    if perk_active(player, PerkId.NINJA):
        dodged = (state.rng.rand(caller=_PLAYER_TAKE_DAMAGE_NINJA_CALLER) % 3) == 0
    elif perk_active(player, PerkId.DODGER):
        dodged = (state.rng.rand(caller=_PLAYER_TAKE_DAMAGE_DODGER_CALLER) % 5) == 0

    health_before = float(player.health)
    if not dodged:
        if perk_active(player, PerkId.HIGHLANDER):
            if (state.rng.rand(caller=_PLAYER_TAKE_DAMAGE_HIGHLANDER_CALLER) % 10) == 0:
                player.health = 0.0
        else:
    ...
    if not lethal_hit:
        state.sfx_queue.append(
            _PLAYER_PAIN_SFX[state.rng.rand(caller=_PLAYER_TAKE_DAMAGE_PAIN_SFX_CALLER) % len(_PLAYER_PAIN_SFX)],
        )
        if not was_alive:
        ...
        if not perk_active(player, PerkId.FINAL_REVENGE):
            state.sfx_queue.append(_PLAYER_DEATH_SFX[state.rng.rand(caller=_PLAYER_TAKE_DAMAGE_DEATH_SFX_CALLER) & 1])
    ...
    if not dodged:
        if not perk_active(player, PerkId.UNSTOPPABLE):
            player.heading += float((state.rng.rand(caller=_PLAYER_TAKE_DAMAGE_HEADING_CALLER) % 100) - 50) * 0.04
        ...
        if player.health <= 20.0 and (state.rng.rand(caller=_PLAYER_TAKE_DAMAGE_LOW_HEALTH_CALLER) & 7) == 3:
