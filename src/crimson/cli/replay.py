...
class _ReplayBenchmarkPayload(msgspec.Struct, forbid_unknown_fields=True):
    ...
    render_telemetry: dict[str, object] | None
...
def _render_telemetry_top_tick_payload(entry: object) -> dict[str, int | float]:
    item = cast("Any", entry)
    return {
...
def _render_telemetry_frame_payload(entry: object) -> dict[str, object]:
    item = cast("Any", entry)
    return {
        "frame_index": int(item.frame_index),
...
def cmd_replay_benchmark(
    ...
    render_telemetry_payload: dict[str, object] | None = None
    if benchmark.render_telemetry is not None:
        telemetry_summary = benchmark.render_telemetry.summary
        artifacts_payload: dict[str, str | None] | None = None
