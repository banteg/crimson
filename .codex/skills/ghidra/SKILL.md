...
- **Ghidra** must be installed. On macOS: `brew install --cask ghidra`.
  On Linux, a common install root is `/opt/ghidra` (or `/opt/ghidra_*`).
- **Java** (OpenJDK 17+) must be available

The skill automatically locates `analyzeHeadless` in common install paths. You can
override it with:
- `GHIDRA_HOME=/path/to/ghidra`
- `GHIDRA_HEADLESS=/path/to/support/analyzeHeadless`
