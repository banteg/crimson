...
- **Game over / high score entry (state `7`)**: implemented for Survival/Rush/Typ-o.
  ...
  - Zig now animates game-over/results panels through the native open/close slide timeline, keeps high-score/button hitboxes aligned with the moving panel, routes Play Again, High scores, and Main Menu actions through the native close timeline before dispatch, and returns Back from result-launched high scores to the results screen.
