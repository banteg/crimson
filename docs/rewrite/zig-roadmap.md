...
- native rollback session construction and live smoke validation in
  ...
  including delayed and reordered guest-input smoke cases that force prediction
  correction without resync,
...
- expand `net smoke-rollback` beyond the in-process happy path and delayed
  input/reorder correction into loss, jitter, reconnect, and resync scenarios,
