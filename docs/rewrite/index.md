...
- Rollback netplay is primary (relay + room codes, protocol v5, reconnect/resync hooks); Zig now has delayed-input, reordered-input, dropped-input, repeated-jitter, guest-requested resync, and relay-token guest self-reconnect smoke paths with post-reconnect input continuity, and remaining hardening work is focused on longer stress runs plus lockstep fallback maintenance.
