...
This map documents `projectile_pool` (`0x004926b8`).

Notes:

- Entry size: `0x40` bytes.
- Pool size: `0x60` entries (loop in `projectile_update`).
- Primary writers: `projectile_spawn` (`0x00420440`) and `projectile_update` (`0x00420b90`).
...
- `weapon_ammo_class` (`0x004d7a28`) is consulted during `projectile_update` to gate one of the hit
...
- `projectile_pool + 0x30` (`projectile_damage_pool`) acts like a shared damage pool for
  ...
  into `creature_apply_damage` (`0x004207c0`) as the damage value before
  subtracting the target's health.

Spawn notes:

- When Fire Bullets is active (`player_fire_bullets_timer` / `0x00490bcc > 0`
...
| `0x0f` | Blow Torch | `fx_spawn_particle` | Sets particle flag `1` in `particle_style_id`. |
| `0x10` | HR Flamer | `fx_spawn_particle` | Sets particle flag `2` in `particle_style_id`. |
| `0x0c` | Rocket Launcher | `fx_spawn_secondary_projectile` type `1` | Uses `secondary_projectile_pool` (`0x00495ad8`). |
