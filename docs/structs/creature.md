...
`creature_spawn_template` (`FUN_00430af0`) is a full spawning algorithm, not a
static struct/table. Given `(template_id, pos_xy, heading)` it:

- always allocates and initializes a base creature (`creature_alloc_slot` + spawn init)
- applies template-specific constants via a large switch on `template_id`
- may allocate additional "formation" creatures (linked AI with ring offsets, etc.)
- may configure spawn slots (deferred child spawns driven by `creature_update_all`)
- applies difficulty/hardcore modifiers and other tail fixes at the end

For the behavior breakdown (and the rewrite model), see:
[`docs/crimsonland-exe/creature-spawning.md`](../crimsonland-exe/creature-spawning.md).

The table below only lists the *direct* type/flags assignments extracted from the
switch (useful for labeling). It does not capture randomized parameters,
formation spawns, spawn slots, or tail modifiers.
...
- `src/crimson/spawn_templates.py` is a partial, hand-maintained index (type/flags
  plus a few fixed constants). It is primarily used for labeling and doc generation.
- `src/crimson/spawn_plan.py` is the start of a 1:1 rewrite of the algorithm as a
  pure plan builder (formations, spawn slots, and tail modifiers).
