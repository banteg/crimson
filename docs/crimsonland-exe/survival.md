...
- Stage `8` → `9` when `player_level >= 26`: spawn 4× `SPIDER_PLASMA_SHOOTER_3C` (`0x3c`, SpiderPlasmaShooter) at `(1088, 384 + i*64)` and 4× at `(-64, 384 + i*64)` (heading π).
- Stage `9` → `10` when `player_level >= 32`: spawn the final wave:
  - `SPIDER_BOSS_3A` (`0x3a`, SpiderBoss) at `(1088, 512)` and `(-64, 512)` (heading π)
  - `SPIDER_PLASMA_SHOOTER_3C` (`0x3c`, SpiderPlasmaShooter) at top edge `((384 + i*64), -64)` and bottom edge `((384 + i*64), 1088)` (heading π)
...
- Without Quick Learner, converts the floating sum of player 0 XP plus reward
  back to an integer. This is not always equivalent to separately truncating
  the reward at large XP values.
...
The Python HUD opens the perk menu, records menu generation and selection, and
applies selected perks through the shared command prelude. Automatic picks are
an explicit simulation/testing option. The interactive selection flow is
implemented; see [session ordering](../rewrite/deterministic-step-pipeline.md).
