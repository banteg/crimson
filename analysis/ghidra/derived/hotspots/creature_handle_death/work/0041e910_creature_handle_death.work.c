...

{
  ...
  if (creature_ptr->active != '\0') {
    ...
    if ((((&creature_pool)[creature_id].flags & 8) != 0) &&
      ...
      copy_dst_ptr = &creature_pool + iVar5;
      for (iVar7 = 0x26; iVar7 != 0; iVar7 = iVar7 + -1) {
        uVar2 = copy_src_ptr->_pad0[0];
        uVar3 = copy_src_ptr->_pad0[1];
        uVar4 = copy_src_ptr->_pad0[2];
        copy_dst_ptr->active = copy_src_ptr->active;
        copy_dst_ptr->_pad0[0] = uVar2;
        copy_dst_ptr->_pad0[1] = uVar3;
        copy_dst_ptr->_pad0[2] = uVar4;
        copy_src_ptr = (creature_t *)&copy_src_ptr->phase_seed;
        copy_dst_ptr = (creature_t *)&copy_dst_ptr->phase_seed;
      }
      rng_value = crt_rand();
      (&creature_pool)[iVar5].phase_seed = (float)(rng_value & 0xff);
    ...
    if (0.0 < bonus_freeze_timer) {
      ...
      do {
        iVar7 = crt_rand();
        effect_spawn_freeze_shard(death_pos_ptr,(float)(iVar7 % 0x264) * 0.01);
        iVar5 = iVar5 + -1;
      } while (iVar5 != 0);
      iVar5 = crt_rand();
      effect_spawn_freeze_shatter(death_pos_ptr,(float)(iVar5 % 0x264) * 0.01);
