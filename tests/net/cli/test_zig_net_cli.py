...
def test_zig_net_smoke_rollback_reports_reconnect_then_resync_recovery() -> None:
    ...
    assert payload["host_input_flags"] == 31
    assert payload["guest_input_flags"] == 27
    assert payload["delayed_packets"] == 1
    assert payload["released_packets"] == 1
    assert payload["dropped_packets"] >= 1
    assert payload["host_reconnect_count"] == 1
    ...
    assert payload["host_tick_index"] > payload["resync_snapshot_tick"]
    assert payload["guest_live_tick_index"] > payload["resync_snapshot_tick"]
    assert payload["host_paused_for_reconnect"] is False
    assert payload["guest_paused_for_reconnect"] is False
    assert payload["host_paused_for_resync"] is False
    assert payload["guest_paused_for_resync"] is False
...
def test_zig_net_smoke_rollback_reports_triple_reconnect_then_resync_recovery() -> None:
    payload = _run_zig_net_json(
        [
            "smoke-rollback",
            "--impair",
            "guest-triple-reconnect-resync",
            "--format",
            "json",
        ],
    )
    ...
    assert payload["status"] == "ok"
    assert payload["impairment"] == "guest-triple-reconnect-resync"
    assert payload["host_input_flags"] == 165
    assert payload["guest_input_flags"] == 125
    assert payload["host_tick_index"] == payload["guest_tick_index"]
    assert payload["delayed_packets"] == 1
...
def test_zig_net_smoke_rollback_reports_triple_reconnect_recovery() -> None:
    payload = _run_zig_net_json(
        [
            "smoke-rollback",
            "--impair",
            "guest-triple-reconnect",
            "--format",
            "json",
        ],
    )
    ...
    assert payload["status"] == "ok"
    assert payload["impairment"] == "guest-triple-reconnect"
    assert payload["host_input_flags"] == 23
    assert payload["guest_input_flags"] == 29
    assert payload["host_tick_index"] == payload["guest_tick_index"]
    assert payload["delayed_packets"] == 0
    assert payload["released_packets"] == 0
    assert payload["dropped_packets"] == 0
...
def test_zig_net_smoke_rollback_reports_double_reconnect_recovery() -> None:
    payload = _run_zig_net_json(
        [
            "smoke-rollback",
            "--impair",
            "guest-double-reconnect",
            "--format",
            "json",
        ],
    )
    ...
    assert payload["status"] == "ok"
    assert payload["impairment"] == "guest-double-reconnect"
    assert payload["host_input_flags"] == 17
    assert payload["guest_input_flags"] == 19
    assert payload["host_tick_index"] == payload["guest_tick_index"]
    assert payload["delayed_packets"] == 0
    assert payload["released_packets"] == 0
    assert payload["dropped_packets"] == 0
...
def test_zig_net_smoke_rollback_reports_reconnect_jitter_recovery() -> None:
    payload = _run_zig_net_json(
        [
            "smoke-rollback",
            "--impair",
            "guest-reconnect-bidirectional-jitter-burst",
            "--format",
            "json",
        ],
    )
    ...
    assert payload["status"] == "ok"
    assert payload["impairment"] == "guest-reconnect-bidirectional-jitter-burst"
    assert payload["host_input_flags"] == 43
    assert payload["guest_input_flags"] == 83
    assert payload["host_tick_index"] == payload["guest_tick_index"]
    assert payload["delayed_packets"] == 6
...
def test_zig_net_smoke_rollback_reports_double_reconnect_jitter_recovery() -> None:
    payload = _run_zig_net_json(
        [
            "smoke-rollback",
            "--impair",
            "guest-double-reconnect-bidirectional-jitter-burst",
            "--format",
            "json",
        ],
    )
    ...
    assert payload["status"] == "ok"
    assert payload["impairment"] == "guest-double-reconnect-bidirectional-jitter-burst"
    assert payload["host_input_flags"] == 43
    assert payload["guest_input_flags"] == 83
    assert payload["host_tick_index"] == payload["guest_tick_index"]
    assert payload["delayed_packets"] == 6
