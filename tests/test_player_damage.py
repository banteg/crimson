...
def test_player_take_damage_resets_low_health_timer_on_hit() -> None:
    state = GameplayState()
    player = PlayerState(index=0, pos=Vec2(0.0, 0.0), health=25.0)
...
def test_player_take_damage_does_not_reset_low_health_timer_above_threshold() -> None:
    state = GameplayState()
    player = PlayerState(index=0, pos=Vec2(0.0, 0.0), health=50.0)
...
def test_player_take_damage_decrements_death_timer_on_death_hit() -> None:
    state = GameplayState()
    player = PlayerState(index=0, pos=Vec2(0.0, 0.0), health=5.0, death_timer=16.0)
