...
def test_run_result_from_replay_mode_reads_authoritative_quest_runtime(monkeypatch) -> None:
    monkeypatch.setattr(
        "crimson.sim.driver.replay_benchmark.player0_shots",
        lambda _state: (7, 3),
    )
    ...
    assert result.game_mode_id == GameMode.QUESTS
    assert result.ticks == 88
    assert result.elapsed_ms == 999
    assert result.score_xp == 456
    assert result.creature_kill_count == 12
    assert result.most_used_weapon_id == WeaponId.PISTOL
    assert result.shots_fired == 7
    assert result.shots_hit == 3
    assert result.rng_state == 12345
