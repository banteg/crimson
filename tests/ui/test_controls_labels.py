...
def _config(**updates: object) -> CrimsonConfig:
...
def test_controls_method_labels_reads_player_arrays() -> None:
    ...
    assert controls_method_labels(config, player_index=0) == ("Mouse relative", "Static")
    assert controls_method_labels(config, player_index=1) == ("Joystick", "Mouse point click")
    assert controls_method_labels(config, player_index=2) == ("Dual Action Pad", "Computer")
    assert controls_method_labels(config, player_index=3) == ("Computer", "Relative")
    assert controls_method_values(config, player_index=1) == (AimScheme.JOYSTICK, MovementControlType.MOUSE_POINT_CLICK)


def test_controls_method_labels_defaults_missing_blob() -> None:
    assert controls_method_labels(_config(), player_index=0) == ("Mouse", "Static")
...
def test_controls_method_values_unknown_move_mode_maps_to_unknown_enum() -> None:
    ...
    assert controls_method_values(config, player_index=0) == (AimScheme.MOUSE, MovementControlType.UNKNOWN)
