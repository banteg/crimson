...
def test_projectile_pool_keeps_flight_timer_when_in_bounds() -> None:
    ...
    assert proj.active
    assert proj.life_timer == 0.4
    assert_float_close(proj.pos.x, 10.0)
...
def test_projectile_pool_update_moves_by_projectile_meta() -> None:
    ...
    assert proj.active
    assert proj.life_timer == 0.4
    assert_float_close(proj.pos.x, 90.0)
...
def test_projectile_pool_update_applies_distance_scaled_damage() -> None:
    ...
    assert_float_close(creatures[0].hp, 33.5)
...
def test_projectile_pool_update_applies_rng_jitter_to_hit_position() -> None:
    ...
    proj = pool.entries[idx]
    assert_float_close(proj.pos.x, 62.0)
    assert proj.life_timer == 0.25

    expected_damage = _expected_damage(62.0, 1.0)
    assert_float_close(creatures[0].hp, 100.0 - expected_damage)
...
def test_projectile_pool_update_type_0x0b_does_not_splash_damage() -> None:
    ...
    assert_float_close(creatures[0].hp, 100.0 - _expected_damage(60.0))
    assert_float_close(creatures[1].hp, 100.0)
    assert_float_close(creatures[2].hp, 100.0)
    assert_float_close(creatures[3].hp, 100.0)
...
def test_projectile_life_timer_f32_decay_delays_deactivate_by_one_tick() -> None:
    ...
    assert_float_close(float(proj.life_timer), -0.09999998658895493)
...
def test_projectile_pool_update_ion_hit_spawns_ring_and_burst_effects() -> None:
    ...
    assert_float_close(float(ring.pos.x), float(proj.pos.x))
    assert_float_close(float(ring.pos.y), float(proj.pos.y))
    assert_float_close(float(ring.half_width), 4.0)
    assert_float_close(float(ring.half_height), 4.0)
    assert_float_close(float(ring.scale_step), 67.5)
    assert_float_close(float(ring.lifetime), 0.08)

    burst = bursts[0]
    assert_float_close(float(burst.half_width), 20.48)
    assert_float_close(float(burst.half_height), 20.48)
    assert_float_close(float(burst.lifetime), 0.448)
...
def test_projectile_pool_update_owner_collision_blocks_later_candidates() -> None:
    ...
    assert hits == []
    assert_float_close(creatures[0].hp, 100.0)
    assert_float_close(creatures[1].hp, 100.0)
...
def test_projectile_pool_update_player_hit_does_not_break_step_loop() -> None:
    ...
    assert hits == []
    assert_float_close(players[0].health, 60.0)
    assert_float_close(proj.life_timer, 0.25)
...
def test_projectile_pool_update_clears_unit_damage_pool_even_when_already_hit_state() -> None:
    ...
    assert_float_close(proj.life_timer, 0.25)
    assert_float_close(proj.damage_pool, 0.0)
...
def test_projectile_pool_emits_hit_event_and_enters_hit_state() -> None:
    ...
    assert_float_close(creatures[0].hp, 82.0)
    assert proj.life_timer == 0.25
    ...
    assert_float_close(proj.life_timer, 0.15)
...
def test_projectile_pool_demo_type_0x0b_does_not_splash_nearby_creatures() -> None:
    ...
    assert_float_close(creatures[0].hp, 68.0)
    assert_float_close(creatures[1].hp, 100.0)
    assert_float_close(creatures[2].hp, 100.0)
...
def test_projectile_pool_demo_ion_minigun_linger_deals_aoe_damage() -> None:
    ...
    assert_float_close(creatures[0].hp, 95.0)
    ...
    assert_float_close(creatures[0].hp, 91.0)
...
def test_secondary_projectile_pool_pulse_switches_to_detonation() -> None:
    ...
    assert_float_close(entry.detonation_t, 0.0)
    assert_float_close(entry.detonation_scale, 0.25)
    assert_float_close(entry.trail_timer, 0.0)
...
def test_secondary_projectile_pool_timeout_switches_to_generic_detonation() -> None:
    ...
    assert entry.vel == Vec2()
    assert_float_close(entry.detonation_t, 0.0)
    assert_float_close(entry.detonation_scale, 0.5)
...
def test_secondary_projectile_pool_type1_accelerates_and_counts_down() -> None:
    ...
    assert_float_close(entry.pos.y, -9.0)

    # Seeker Rockets (type 1): accelerate by factor (1.0 + dt * 3.0) while speed < 500.
    assert_float_close(entry.vel.y, -117.0)
    assert_float_close(entry.speed, float(f32(2.0 - 0.1)))

    # No further acceleration once past the 500 speed threshold.
    entry.vel = Vec2(0.0, -600.0)
    pool.update_pulse_gun(0.1, [])
    assert_float_close(entry.vel.y, -600.0)
...
def test_secondary_projectile_pool_type1_hit_switches_to_detonation_scale() -> None:
    ...
    assert entry.vel == Vec2()
    assert_float_close(entry.detonation_t, 0.0)
    assert_float_close(entry.detonation_scale, 1.0)
...
def test_secondary_projectile_pool_hit_queues_sfx_and_fx() -> None:
    ...
    assert entry.vel == Vec2()
    assert_float_close(entry.detonation_t, 0.0)
    assert_float_close(entry.detonation_scale, 0.35)
...
def test_secondary_projectile_pool_direct_hit_passes_impulse() -> None:
    ...
    assert_float_close(impulse_y, -1170.0)
...
def test_secondary_projectile_pool_detonation_aoe_passes_impulse() -> None:
    ...
    assert len(calls) == 1
    impulse_x, impulse_y = calls[0]
    assert_float_close(impulse_x, 0.06)
    assert_float_close(impulse_y, 0.08)
...
def test_secondary_projectile_pool_freeze_spawns_extra_shards_and_burst() -> None:
    ...
    for _, y in runtime.effects.calls[:4]:
        assert_float_close(y, -9.0)
    for _, y in runtime.effects.calls[4:]:
        assert_float_close(y, -4.0)
