...
def test_demo_mode_does_not_apply_movement_deadzone() -> None:
    ...
    player_normal = PlayerState(index=0, pos_x=512.0, pos_y=512.0)
    player_update(player_normal, input_state, dt=1.0, state=normal)
    assert player_normal.pos_x == 512.0
    assert player_normal.pos_y == 512.0

    demo = GameplayState()
    demo.demo_mode_active = True
    player_demo = PlayerState(index=0, pos_x=512.0, pos_y=512.0)
    player_update(player_demo, input_state, dt=1.0, state=demo)
    assert player_demo.pos_x > 512.0
