...
def test_capture_creature_spawn_event_applies_added_head_overrides() -> None:
    ...
    assert_float_close(float(creature.heading), 1.1278764009475708, abs_tol=1e-6)
    assert_float_close(float(creature.target_heading), 0.621416449546814, abs_tol=1e-6)
    assert int(creature.ai_mode) == 3
    assert int(creature.link_index) == 0
    assert_float_close(float(creature.hp), 123.5, abs_tol=1e-6)
    assert_float_close(float(creature.hitbox_size), 9.5, abs_tol=1e-6)
    assert_float_close(float(creature.orbit_angle), 0.25, abs_tol=1e-6)
    assert_float_close(float(creature.orbit_radius), 0.75, abs_tol=1e-6)
    assert int(creature.flags) == 17
    assert int(creature.type_id) == 7
    assert_float_close(float(creature.pos.x), 12.25, abs_tol=1e-6)
    assert_float_close(float(creature.pos.y), 34.5, abs_tol=1e-6)
...
def test_capture_creature_spawn_event_applies_added_head_without_spawn_rows() -> None:
    ...
    assert creature.active
    assert_float_close(float(creature.heading), 0.28999999165534973, abs_tol=1e-6)
    assert_float_close(float(creature.target_heading), 0.521416425704956, abs_tol=1e-6)
    assert int(creature.ai_mode) == 0
    assert int(creature.link_index) == 1
    assert_float_close(float(creature.orbit_radius), 1.25, abs_tol=1e-6)
...
def test_quest_runner_disables_world_dt_steps_for_original_capture_dt_overrides() -> None:
    ...
    assert_float_close(override_with_perk[0], override_without_perk[0], abs_tol=1e-6)
    assert_float_close(override_with_perk[1], override_without_perk[1], abs_tol=1e-6)
...
def test_quest_runner_resets_run_on_capture_state_transition_to_12() -> None:
    ...
    assert_float_close(float(player.ammo), 11.0, abs_tol=1e-6)
...
def test_survival_runner_bootstrap_player_shot_cooldown_blocks_first_tick_fire() -> None:
    ...
    assert_float_close(ammo_with_shot_cooldown, 12.0, abs_tol=1e-6)
...
def test_survival_runner_bootstrap_perk_counts_enable_alternate_weapon_swap() -> None:
    ...
    assert_float_close(ammo_without_perk, 0.0, abs_tol=1e-6)
    assert weapon_with_perk == 1
    assert_float_close(ammo_with_perk, 12.0, abs_tol=1e-6)
