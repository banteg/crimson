...
def test_ai_mode_0_orbits_when_close() -> None:
    ...
    ai = creature_ai_update_target(c, player_x=100.0, player_y=0.0, creatures=[c], dt=1.0 / 60.0)
...
def test_ai_mode_5_scales_down_near_link() -> None:
    ...
    ai = creature_ai_update_target(c, player_x=0.0, player_y=0.0, creatures=[link, c], dt=1.0 / 60.0)
    assert c.force_target == 0
...
def test_ai_mode_4_link_dead_self_damage() -> None:
    ...
    ai = creature_ai_update_target(c, player_x=100.0, player_y=0.0, creatures=[dead, c], dt=1.0 / 60.0)
    assert c.ai_mode == 0
...
def test_ai_mode_7_orbit_radius_timer_counts_down() -> None:
    ...
    ai = creature_ai_update_target(c, player_x=100.0, player_y=0.0, creatures=[c], dt=0.5)
    assert ai.self_damage is None
    assert c.ai_mode == 7
