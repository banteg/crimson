...
from crimson.sim.input_providers import NetworkInputProvider, normalize_provider_tick_inputs
...
def test_network_provider_stalls_when_runtime_frame_missing() -> None:
    ...
    assert provider.pull_tick_input(10) is None
...
def test_network_provider_uses_runtime_resolved_order_without_remerge() -> None:
    ...
    normalized = provider.pull_tick_input(5)

    assert normalized is not None
    assert normalized[0].move.x == pytest.approx(3.0)
    assert normalized[1].move.x == pytest.approx(4.0)
