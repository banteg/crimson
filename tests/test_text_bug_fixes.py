...
def test_weapon_display_name_fixes_spelling_by_default() -> None:
    plague_weapon_id = int(WeaponId.PLAGUE_SPREADER_GUN)
    lightning_weapon_id = int(WeaponId.LIGHTNING_RIFLE)
...
def test_bonus_label_for_entry_uses_typo_fixes_unless_preserving_bugs() -> None:
    entry = BonusEntry(bonus_id=int(BonusId.WEAPON), amount=int(WeaponId.LIGHTNING_RIFLE))
