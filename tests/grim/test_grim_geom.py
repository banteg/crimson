...
def test_vec2_length_and_length_sq() -> None:
    ...
    assert_float_close(vec.length_sq(), 25.0)
    assert_float_close(vec.length(), 5.0)
...
def test_vec2_angle_helpers_round_trip() -> None:
    ...
    assert_float_close(vec.to_angle(), angle)
...
def test_vec2_polar_helpers_round_trip() -> None:
    ...
    assert_float_close(polar_angle, angle)
    assert_float_close(polar_radius, radius)
...
def test_vec2_heading_helpers_round_trip() -> None:
    ...
    assert_float_close(vec.to_heading(), heading)
...
def test_vec2_rotated() -> None:
    ...
    assert_float_close(rotated.x, 0.0)
    assert_float_close(rotated.y, 1.0)
...
def test_vec2_clamp_rect() -> None:
    ...
    assert_float_close(clamped.x, 0.0)
    assert_float_close(clamped.y, 8.0)
...
def test_vec2_distance_sq() -> None:
    ...
    assert_float_close(Vec2.distance_sq(a, b), 25.0)
...
def test_vec2_distance_to() -> None:
    ...
    assert_float_close(a.distance_to(b), 5.0)
...
def test_vec2_direction_to() -> None:
    ...
    assert_float_close(direction.length(), 1.0)
    assert_float_close(direction.x, 0.6)
    assert_float_close(direction.y, 0.8)
...
def test_vec2_lerp() -> None:
    ...
    assert_float_close(lerped.x, 2.0)
    assert_float_close(lerped.y, 4.0)
...
def test_vec2_component_helpers() -> None:
    ...
    assert_float_close(a.avg_component(), 7.0)
...
def test_vec2_to_rl() -> None:
    ...
    assert_float_close(float(result.x), 1.25)
    assert_float_close(float(result.y), -4.5)


def test_rect_properties_and_helpers() -> None:
    rect = Rect(10.0, 20.0, 30.0, 40.0)

    assert_float_close(rect.left, 10.0)
    assert_float_close(rect.top, 20.0)
    ...
    assert_float_close(rect.right, 40.0)
    assert_float_close(rect.bottom, 60.0)
