...
def test_grim_data_manifest_applies_typed_data_tranche() -> None:
    ...
    assert payload["summary"]["source_entry_count"] == 351
    assert payload["summary"]["code_label_entries"] == 3
    assert payload["summary"]["entry_count"] == 348
    assert payload["summary"]["typed_entries"] == 298
    assert payload["summary"]["explicit_size_entries"] == 314
    assert payload["summary"]["explicit_alignment_entries"] == 314
    assert payload["summary"]["explicit_initializer_entries"] == 314
    assert payload["summary"]["fully_specified_entries"] == 314
