...
def test_advance_survival_spawn_stage_stage2_grid_positions() -> None:
    ...
    x0, y0 = spawns[0].pos
    assert x0 == pytest.approx(1088.0, abs=1e-9)
    assert y0 == pytest.approx(256.0, abs=1e-9)

    x_last, y_last = spawns[-1].pos
    assert x_last == pytest.approx(1088.0, abs=1e-9)
    assert y_last == pytest.approx(256.0 + 11.0 * (128.0 / 3.0), abs=1e-9)
...
def test_advance_survival_spawn_stage_stage9_final_wave() -> None:
    ...
    assert spawns[0].pos == (pytest.approx(1088.0, abs=1e-9), pytest.approx(512.0, abs=1e-9))
    assert spawns[1].pos == (pytest.approx(-64.0, abs=1e-9), pytest.approx(512.0, abs=1e-9))

    top = spawns[2:6]
    bottom = spawns[6:10]
    assert {s.template_id for s in top} == {0x3C}
    assert {s.template_id for s in bottom} == {0x3C}
    for _, y in (s.pos for s in top):
        assert y == pytest.approx(-64.0, abs=1e-9)
    for _, y in (s.pos for s in bottom):
