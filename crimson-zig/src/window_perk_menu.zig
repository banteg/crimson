...
pub fn drawMenu(
    ...
    for (choices, 0..) |perk_id, idx| {
        const label = game_ids.perkDisplayName(perk_id, runner.session.gore_disabled, runner.session.state.preserve_bugs);
    ...
    const desc = game_ids.perkDisplayDescription(selected, runner.session.gore_disabled, runner.session.state.preserve_bugs);
