...
pub const IngestResult = struct {
    messages: std.ArrayList(lockstep_protocol.NetMessage) = .empty,
    duplicate: bool = false,

    pub fn deinit(self: *IngestResult, allocator: std.mem.Allocator) void {
        for (self.messages.items) |*message| lockstep_protocol.deinitMessage(allocator, message);
        self.messages.deinit(allocator);
        self.* = undefined;
    }
};

pub const LockstepReliableLink = struct {
    resend_ms: i64 = lockstep_protocol.reliable_resend_ms,
    next_seq: i32 = 1,
    recv_highest_seq: i32 = 0,
    pending: std.ArrayList(PendingReliable) = .empty,
    recv_buffer: std.ArrayList(lockstep_protocol.LockstepPacket) = .empty,
    rtt_last_ms: i64 = 0,
    rtt_ewma_ms: f64 = 0.0,
    resend_count: i64 = 0,

    pub fn deinit(self: *LockstepReliableLink, allocator: std.mem.Allocator) void {
        for (self.pending.items) |*pending| pending.deinit(allocator);
        self.pending.deinit(allocator);
        for (self.recv_buffer.items) |*packet| lockstep_protocol.deinitPacket(allocator, packet);
        self.recv_buffer.deinit(allocator);
        self.* = undefined;
    }

    pub fn pendingCount(self: *const LockstepReliableLink) usize {
        return self.pending.items.len;
    }

    pub fn primeRecvSeq(self: *LockstepReliableLink, seq: i32) void {
        if (seq <= self.recv_highest_seq) return;
        self.recv_highest_seq = seq;
    }

    pub fn buildPacket(
        self: *LockstepReliableLink,
        ...
        const packet: lockstep_protocol.LockstepPacket = .{
            .seq = seq,
    ...
    pub fn ingestPacket(
        ...
        if (!packet.reliable) {
            var owned_message = try lockstep_protocol.cloneMessage(allocator, packet.message);
            errdefer lockstep_protocol.deinitMessage(allocator, &owned_message);
            try result.messages.append(allocator, owned_message);
            owned_message = .{ .keep_alive = .{} };
            return result;
        }
        ...
        while (self.findRecvBufferIndex(next_seq)) |idx| {
            ...
            errdefer lockstep_protocol.deinitPacket(allocator, &next_packet);
            try result.messages.append(allocator, next_packet.message);
            next_packet.message = .{ .keep_alive = .{} };
    ...
    pub fn pollResends(self: *LockstepReliableLink, allocator: std.mem.Allocator, now_ms: i64) !std.ArrayList(lockstep_protocol.LockstepPacket) {
        var out: std.ArrayList(lockstep_protocol.LockstepPacket) = .empty;
        errdefer {
            for (out.items) |*packet| lockstep_protocol.deinitPacket(allocator, packet);
        ...
        for (self.pending.items) |*pending| {
            ...
            var packet = try lockstep_protocol.clonePacket(allocator, pending.packet);
            errdefer lockstep_protocol.deinitPacket(allocator, &packet);
    ...
    fn applyAck(self: *LockstepReliableLink, allocator: std.mem.Allocator, ack: i32, now_ms: i64) void {
        if (ack <= 0) return;
    ...
    fn findRecvBufferIndex(self: *const LockstepReliableLink, seq: i32) ?usize {
...
pub fn deinitPacketList(allocator: std.mem.Allocator, packets: *std.ArrayList(lockstep_protocol.LockstepPacket)) void {
    for (packets.items) |*packet| lockstep_protocol.deinitPacket(allocator, packet);
    packets.deinit(allocator);
    packets.* = .empty;
}

test "lockstep reliable packet is acked and removed" {
    const allocator = std.testing.allocator;
    var sender: LockstepReliableLink = .{ .resend_ms = 40 };
    defer sender.deinit(allocator);
    var receiver: LockstepReliableLink = .{ .resend_ms = 40 };
    ...
    const ack = try receiver.buildPacket(allocator, .{ .keep_alive = .{} }, false, 1001);
...
test "lockstep duplicate reliable packet is dropped" {
    const allocator = std.testing.allocator;
    var receiver: LockstepReliableLink = .{ .resend_ms = 40 };
    defer receiver.deinit(allocator);
    var sender: LockstepReliableLink = .{ .resend_ms = 40 };
...
test "lockstep reliable packet is resent after timeout" {
    const allocator = std.testing.allocator;
    var sender: LockstepReliableLink = .{ .resend_ms = 40 };
    defer sender.deinit(allocator);

    _ = try sender.buildPacket(allocator, .{ .welcome = .{
        .accepted = true,
        .session_id = "session",
        .build_id = "0.1.0",
    } }, true, 0);
...
test "lockstep reliable delivery buffers out of order packets and acks contiguously" {
    const allocator = std.testing.allocator;
    var sender: LockstepReliableLink = .{ .resend_ms = 40 };
    defer sender.deinit(allocator);
    var receiver: LockstepReliableLink = .{ .resend_ms = 40 };
...
test "lockstep reliable pending packets own lobby state payloads" {
    const allocator = std.testing.allocator;
    var sender: LockstepReliableLink = .{ .resend_ms = 40 };
    defer sender.deinit(allocator);
    ...
    _ = try sender.buildPacket(allocator, .{ .lobby_state = .{
        .session_id = "session",
        .slots = slots[0..],
    } }, true, 0);
...
test "lockstep reliable buffers decoded out of order payloads after decode storage is freed" {
    const allocator = std.testing.allocator;
    var receiver: LockstepReliableLink = .{ .resend_ms = 40 };
    defer receiver.deinit(allocator);

    const encoded = try lockstep_protocol.encodePacket(allocator, .{
        .seq = 2,
        .ack = 0,
        .reliable = true,
    ...
    var first = try receiver.ingestPacket(allocator, .{
        ...
        .ack = 0,
        .reliable = true,
        .message = .{ .keep_alive = .{ .tick_index = 7 } },
    }, 1);
    defer first.deinit(allocator);
    try std.testing.expectEqual(@as(usize, 2), first.messages.items.len);
    switch (first.messages.items[1]) {
...
test "lockstep non-reliable decoded payload is cloned for delivered result" {
    const allocator = std.testing.allocator;
    var receiver: LockstepReliableLink = .{ .resend_ms = 40 };
    defer receiver.deinit(allocator);
    ...
    var delivered: IngestResult = undefined;
    {
        const decoded = try lockstep_protocol.decodePacket(allocator, encoded);
        defer decoded.deinit();
        delivered = try receiver.ingestPacket(allocator, decoded.value, 0);
    }
    defer delivered.deinit(allocator);
...
test "lockstep reliable link can prime receive sequence" {
    const allocator = std.testing.allocator;
    var receiver: LockstepReliableLink = .{ .resend_ms = 40 };
    defer receiver.deinit(allocator);
    receiver.primeRecvSeq(4);
    receiver.primeRecvSeq(3);
    try std.testing.expectEqual(@as(i32, 4), receiver.recv_highest_seq);

    var duplicate = try receiver.ingestPacket(allocator, .{
        .seq = 4,
        .ack = 0,
        .reliable = true,
        .message = .{ .ready = .{ .slot_index = 1, .ready = true } },
    }, 0);
    defer duplicate.deinit(allocator);
    try std.testing.expect(duplicate.duplicate);
    try std.testing.expectEqual(@as(usize, 0), duplicate.messages.items.len);
}
