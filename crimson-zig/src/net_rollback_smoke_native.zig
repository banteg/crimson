...
const Impairment = enum {
    ...
    guest_double_reconnect_resync,
    guest_triple_reconnect_resync,
    guest_double_reconnect,
    guest_triple_reconnect,
    jitter_burst,
    ...
    fn label(self: Impairment) []const u8 {
        return switch (self) {
            ...
            .guest_double_reconnect_resync => "guest-double-reconnect-resync",
            .guest_triple_reconnect_resync => "guest-triple-reconnect-resync",
            .guest_double_reconnect => "guest-double-reconnect",
            .guest_triple_reconnect => "guest-triple-reconnect",
...
fn runSmoke(allocator: std.mem.Allocator, io: Io, impairment: Impairment) !SmokePayload {
    const force_guest_resync = impairment == .force_guest_resync or impairment == .guest_reconnect_resync or impairment == .guest_double_reconnect_resync or impairment == .guest_triple_reconnect_resync;
    const guest_reconnect = impairment == .guest_reconnect or impairment == .guest_reconnect_resync or impairment == .guest_double_reconnect_resync or impairment == .guest_triple_reconnect_resync or impairment == .guest_double_reconnect or impairment == .guest_triple_reconnect or impairment == .guest_double_reconnect_bidirectional_jitter_burst or impairment == .guest_triple_reconnect_bidirectional_jitter_burst;
    ...
    if (impairment == .guest_double_reconnect_resync) {
        return runGuestDoubleReconnectResyncSmoke(
            allocator,
            io,
            server,
            &service,
    ...
    if (impairment == .guest_triple_reconnect_resync) {
        return runGuestTripleReconnectResyncSmoke(
            allocator,
            io,
            server,
            &service,
    ...
    if (impairment == .guest_double_reconnect) {
        return runGuestDoubleReconnectSmoke(
            allocator,
            io,
            server,
            &service,
    ...
    if (impairment == .guest_triple_reconnect) {
        return runGuestTripleReconnectSmoke(
            allocator,
            io,
            server,
            &service,
    ...
    if (impairment == .guest_reconnect_bidirectional_jitter_burst) {
        return runGuestReconnectBidirectionalJitterBurstSmoke(
            allocator,
            io,
            server,
            &service,
    ...
    if (impairment == .guest_double_reconnect_bidirectional_jitter_burst) {
        return runGuestDoubleReconnectBidirectionalJitterBurstSmoke(
            allocator,
            io,
            server,
            &service,
    ...
    switch (impairment) {
        ...
        .guest_double_reconnect_resync => unreachable,
        .guest_triple_reconnect_resync => unreachable,
        .guest_double_reconnect => unreachable,
        .guest_triple_reconnect => unreachable,
...
fn runGuestDoubleReconnectSmoke(
    allocator: std.mem.Allocator,
    io: Io,
    server: relay_transport.UdpTransport,
    service: *relay_service.RelayService,
    ...
    const host_runtime = &(host.session.runtime orelse return error.RollbackRuntimeMissing);
    const guest_runtime = &(guest.session.runtime orelse return error.RollbackRuntimeMissing);
    if (host_runtime.reconnect_count != 2 or guest_runtime.reconnect_count != 2) return error.RollbackReconnectCountMismatch;
    ...
    return .{
        ...
        .guest_port = guest.boundPort(),
        .room_code = code,
        .impairment = Impairment.guest_double_reconnect.label(),
        .packets_sent = packets_sent,
        .delayed_packets = 0,
        .released_packets = 0,
        .dropped_packets = 0,
...
fn runGuestTripleReconnectSmoke(
    allocator: std.mem.Allocator,
    io: Io,
    server: relay_transport.UdpTransport,
    service: *relay_service.RelayService,
    ...
    return .{
        ...
        .guest_port = guest.boundPort(),
        .room_code = code,
        .impairment = Impairment.guest_triple_reconnect.label(),
        .packets_sent = packets_sent,
        .delayed_packets = 0,
        .released_packets = 0,
        .dropped_packets = 0,
...
fn runGuestReconnectBidirectionalJitterBurstSmoke(
    allocator: std.mem.Allocator,
    io: Io,
    server: relay_transport.UdpTransport,
    service: *relay_service.RelayService,
    ...
    var packets_sent = initial_packets_sent;
    packets_sent += try forceGuestReconnectCycle(allocator, io, server, service, host, guest, code, guest_slot, 1800, 7000, 1);
    const jitter = try driveBidirectionalJitterBurst(allocator, io, server, service, host, guest, 7600, false);
    packets_sent += jitter.packets_sent;

    const host_runtime = &(host.session.runtime orelse return error.RollbackRuntimeMissing);
    const guest_runtime = &(guest.session.runtime orelse return error.RollbackRuntimeMissing);
    if (host_runtime.reconnect_count != 1 or guest_runtime.reconnect_count != 1) return error.RollbackReconnectCountMismatch;
    if (host_runtime.resync_count != 0 or guest_runtime.resync_count != 0) return error.RollbackUnexpectedResyncPause;
    if (host_runtime.paused_for_reconnect or guest_runtime.paused_for_reconnect) return error.RollbackReconnectIncomplete;
    ...
    return .{
        ...
        .guest_port = guest.boundPort(),
        .room_code = code,
        .impairment = Impairment.guest_reconnect_bidirectional_jitter_burst.label(),
        .packets_sent = packets_sent,
...
fn runGuestDoubleReconnectBidirectionalJitterBurstSmoke(
    allocator: std.mem.Allocator,
    io: Io,
    server: relay_transport.UdpTransport,
    service: *relay_service.RelayService,
    ...
    const jitter = try driveBidirectionalJitterBurst(allocator, io, server, service, host, guest, 14600, false);
    packets_sent += jitter.packets_sent;
...
fn runGuestDoubleReconnectResyncSmoke(
    allocator: std.mem.Allocator,
    io: Io,
    server: relay_transport.UdpTransport,
    service: *relay_service.RelayService,
    ...
    last_host_step = exchange.host_step;
    last_guest_step = exchange.guest_step;
    if (last_host_step.last_tick_index != last_guest_step.last_tick_index) return error.RollbackReconnectTickMismatch;
    if (host_runtime.reconnect_count != 2 or guest_runtime.reconnect_count != 2) return error.RollbackReconnectCountMismatch;

    for (0..5) |idx| {
        const tick: u32 = @intCast(idx + 1);
        const now_ms: i64 = 16000 + @as(i64, @intCast(idx)) * 20;
    ...
    if (packet_impairment.delayed_packets != 1 or packet_impairment.dropped_packets == 0) return error.RollbackImpairmentNotApplied;
    packets_sent += try packet_impairment.releaseDelayed(allocator, io, server);
    try driveGuestUntilResyncPaused(allocator, io, guest, 16120);
    packets_sent += try driveGuestResyncUntilComplete(allocator, io, server, service, host, guest, 16122);
    if (host_runtime.resync_count != 0) return error.RollbackHostRequestedResync;
    if (guest_runtime.paused_for_resync or guest_runtime.pending_resync_snapshot != null) return error.RollbackGuestResyncIncomplete;
    if (guest.runner == null) return error.RollbackRunnerMissing;
    const snapshot_tick: i32 = @intCast(guest.runner.?.session.tick_index -| 1);
    if (snapshot_tick < 4) return error.RollbackSnapshotNotApplied;
    if (host_runtime.resync_count != 0 or guest_runtime.resync_count != 1) return error.RollbackGuestResyncIncomplete;
    ...
    return .{
        ...
        .guest_port = guest.boundPort(),
        .room_code = code,
        .impairment = Impairment.guest_double_reconnect_resync.label(),
        .packets_sent = packets_sent,
        .delayed_packets = packet_impairment.delayed_packets,
        .released_packets = packet_impairment.released_packets,
        .dropped_packets = packet_impairment.dropped_packets,
...
fn runGuestTripleReconnectResyncSmoke(
    allocator: std.mem.Allocator,
    io: Io,
    server: relay_transport.UdpTransport,
    service: *relay_service.RelayService,
    ...
    for (0..5) |idx| {
        const tick: u32 = @intCast(idx + 1);
        const now_ms: i64 = 23000 + @as(i64, @intCast(idx)) * 20;
    ...
    if (packet_impairment.delayed_packets != 1 or packet_impairment.dropped_packets == 0) return error.RollbackImpairmentNotApplied;
    packets_sent += try packet_impairment.releaseDelayed(allocator, io, server);
    try driveGuestUntilResyncPaused(allocator, io, guest, 23120);
    packets_sent += try driveGuestResyncUntilComplete(allocator, io, server, service, host, guest, 23122);
    if (host_runtime.resync_count != 0) return error.RollbackHostRequestedResync;
    if (guest_runtime.paused_for_resync or guest_runtime.pending_resync_snapshot != null) return error.RollbackGuestResyncIncomplete;
    if (guest.runner == null) return error.RollbackRunnerMissing;
    const snapshot_tick: i32 = @intCast(guest.runner.?.session.tick_index -| 1);
    ...
    return .{
        ...
        .guest_port = guest.boundPort(),
        .room_code = code,
        .impairment = Impairment.guest_triple_reconnect_resync.label(),
        .packets_sent = packets_sent,
        .delayed_packets = packet_impairment.delayed_packets,
        .released_packets = packet_impairment.released_packets,
        .dropped_packets = packet_impairment.dropped_packets,
...
fn runBidirectionalJitterBurstSmoke(
    ...
    const jitter = try driveBidirectionalJitterBurst(allocator, io, server, service, host, guest, 1520, true);
    const packets_sent = initial_packets_sent + jitter.packets_sent;
    const host_runtime = &(host.session.runtime orelse return error.RollbackRuntimeMissing);
    const guest_runtime = &(guest.session.runtime orelse return error.RollbackRuntimeMissing);
...
fn driveBidirectionalJitterBurst(
    allocator: std.mem.Allocator,
    io: Io,
    server: relay_transport.UdpTransport,
    service: *relay_service.RelayService,
    ...
    guest: *rollback_live_session.LiveSession,
    start_ms: i64,
    require_rollback_corrections: bool,
) !JitterBurstSummary {
    var summary: JitterBurstSummary = .{};
    ...
    for (0..3) |idx| {
        const tick: u32 = @intCast(idx + 1);
        const now_ms: i64 = start_ms + @as(i64, @intCast(idx)) * 30;
        ...
        if (summary.host_step.frames_advanced == 0) return error.RollbackJitterHostFrameMissing;
        if (summary.host_step.last_input_flags[1] == guest_flags) return error.RollbackImpairmentDidNotForcePrediction;
        ...
        if (summary.guest_step.last_input_flags[0] == host_flags) return error.RollbackImpairmentDidNotForcePrediction;
    ...
    const host_runtime = &(host.session.runtime orelse return error.RollbackRuntimeMissing);
    const guest_runtime = &(guest.session.runtime orelse return error.RollbackRuntimeMissing);
    summary.delayed_packets = guest_to_host.delayed_packets + host_to_guest.delayed_packets;
    summary.released_packets = guest_to_host.released_packets + host_to_guest.released_packets;
    summary.dropped_packets = guest_to_host.dropped_packets + host_to_guest.dropped_packets;
    if (summary.delayed_packets != 6 or summary.released_packets != 6 or summary.dropped_packets != 0) return error.RollbackImpairmentNotApplied;
    if (host_runtime.resync_count != 0 or guest_runtime.resync_count != 0) return error.RollbackUnexpectedResyncPause;
    if (host_runtime.paused_for_resync or guest_runtime.paused_for_resync) return error.RollbackUnexpectedResyncPause;
...
fn driveLiveInputExchange(
    ...
    for (0..32) |attempt| {
        const step_ms = now_ms + 4 + @as(i64, @intCast(attempt)) * 20;
...
fn forceGuestReconnectCycle(
    ...
    var packets_sent: usize = 0;
    try markServicePeerLastSeen(service, code, host.session.local_slot_index, disconnect_ms);
    try markServicePeerLastSeen(service, code, guest_slot, 0);
    var pruned = try service.pruneTimeouts(allocator, disconnect_ms, 1000);
    defer pruned.deinit(allocator);
    packets_sent += try flushAddressedOutbox(allocator, io, server, pruned);
    packets_sent += try driveHostUntilReconnectPause(allocator, io, server, service, host, disconnect_ms + 1);
    ...
    const guest_runtime = &(guest.session.runtime orelse return error.RollbackRuntimeMissing);
    if (host_runtime.paused_for_reconnect or guest_runtime.paused_for_reconnect) return error.RollbackReconnectIncomplete;
    if (host_runtime.reconnect_count != expected_reconnect_count or guest_runtime.reconnect_count != expected_reconnect_count) return error.RollbackReconnectCountMismatch;
    if (host_runtime.reconnect_deadline_ms != 0) return error.RollbackReconnectDeadlineActive;
    packets_sent += try settleLiveQueuesUntilIdle(allocator, io, server, service, host, guest, reconnect_ms + 40);
    return packets_sent;
}

fn driveGuestUntilReconnectPause(
    allocator: std.mem.Allocator,
    io: Io,
    guest: *rollback_live_session.LiveSession,
    start_ms: i64,
...
const PacketImpairment = struct {
    ...
    fn isForceGuestResync(self: *const PacketImpairment) bool {
        return self.mode == .force_guest_resync or self.mode == .guest_reconnect_resync or self.mode == .guest_double_reconnect_resync or self.mode == .guest_triple_reconnect_resync;
    }
...
fn parseArgs(args: []const []const u8) ParseOutcome {
    ...
    while (idx < args.len) : (idx += 1) {
        ...
        } else if (std.mem.eql(u8, arg, "--guest-double-reconnect-resync")) {
            request.impairment = .guest_double_reconnect_resync;
        } else if (std.mem.eql(u8, arg, "--guest-triple-reconnect-resync")) {
            request.impairment = .guest_triple_reconnect_resync;
        } else if (std.mem.eql(u8, arg, "--guest-double-reconnect")) {
            request.impairment = .guest_double_reconnect;
        } else if (std.mem.eql(u8, arg, "--guest-triple-reconnect")) {
            request.impairment = .guest_triple_reconnect;
        ...
        } else if (std.mem.eql(u8, arg, "--guest-reconnect-bidirectional-jitter-burst")) {
...
fn parseImpairment(value: []const u8) ?Impairment {
    ...
    if (std.ascii.eqlIgnoreCase(text, "guest-double-reconnect-resync")) return .guest_double_reconnect_resync;
    if (std.ascii.eqlIgnoreCase(text, "guest-triple-reconnect-resync")) return .guest_triple_reconnect_resync;
    if (std.ascii.eqlIgnoreCase(text, "guest-double-reconnect")) return .guest_double_reconnect;
    if (std.ascii.eqlIgnoreCase(text, "guest-triple-reconnect")) return .guest_triple_reconnect;
...
test "rollback smoke command can resync after guest reconnects twice" {
    const output = try runRollbackSmoke(std.testing.allocator, std.Io.Threaded.global_single_threaded.io(), &.{ "--json", "--impair", "guest-double-reconnect-resync" });
    defer output.deinit(std.testing.allocator);

    try std.testing.expectEqual(@as(u8, 0), output.exit_code);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"impairment\": \"guest-double-reconnect-resync\"") != null);
...
test "rollback smoke command can resync after guest reconnects three times" {
    const output = try runRollbackSmoke(std.testing.allocator, std.Io.Threaded.global_single_threaded.io(), &.{ "--json", "--impair", "guest-triple-reconnect-resync" });
    defer output.deinit(std.testing.allocator);

    try std.testing.expectEqual(@as(u8, 0), output.exit_code);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"impairment\": \"guest-triple-reconnect-resync\"") != null);
...
test "rollback smoke command can reconnect guest twice" {
    const output = try runRollbackSmoke(std.testing.allocator, std.Io.Threaded.global_single_threaded.io(), &.{ "--json", "--impair", "guest-double-reconnect" });
    defer output.deinit(std.testing.allocator);

    try std.testing.expectEqual(@as(u8, 0), output.exit_code);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"impairment\": \"guest-double-reconnect\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"host_input_flags\": 17") != null);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"guest_input_flags\": 19") != null);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"host_reconnect_count\": 2") != null);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"guest_reconnect_count\": 2") != null);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"host_resync_count\": 0") != null);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"guest_resync_count\": 0") != null);
...
test "rollback smoke command can reconnect guest three times" {
    const output = try runRollbackSmoke(std.testing.allocator, std.Io.Threaded.global_single_threaded.io(), &.{ "--json", "--impair", "guest-triple-reconnect" });
    defer output.deinit(std.testing.allocator);

    try std.testing.expectEqual(@as(u8, 0), output.exit_code);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"impairment\": \"guest-triple-reconnect\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"host_input_flags\": 23") != null);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"guest_input_flags\": 29") != null);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"host_reconnect_count\": 3") != null);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"guest_reconnect_count\": 3") != null);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"host_resync_count\": 0") != null);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"guest_resync_count\": 0") != null);
...
test "rollback smoke command can absorb bidirectional jitter after guest reconnect" {
    const output = try runRollbackSmoke(std.testing.allocator, std.Io.Threaded.global_single_threaded.io(), &.{ "--json", "--impair", "guest-reconnect-bidirectional-jitter-burst" });
    defer output.deinit(std.testing.allocator);

    try std.testing.expectEqual(@as(u8, 0), output.exit_code);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"impairment\": \"guest-reconnect-bidirectional-jitter-burst\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"host_input_flags\": 43") != null);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"guest_input_flags\": 83") != null);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"delayed_packets\": 6") != null);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"released_packets\": 6") != null);
...
test "rollback smoke command can absorb bidirectional jitter after guest reconnects twice" {
    const output = try runRollbackSmoke(std.testing.allocator, std.Io.Threaded.global_single_threaded.io(), &.{ "--json", "--impair", "guest-double-reconnect-bidirectional-jitter-burst" });
    defer output.deinit(std.testing.allocator);

    try std.testing.expectEqual(@as(u8, 0), output.exit_code);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"impairment\": \"guest-double-reconnect-bidirectional-jitter-burst\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"delayed_packets\": 6") != null);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"released_packets\": 6") != null);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"dropped_packets\": 0") != null);
    try std.testing.expect(std.mem.indexOf(u8, output.stdout, "\"host_reconnect_count\": 2") != null);
