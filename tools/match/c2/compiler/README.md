...
| [pu-id-delta-profile.md](pu-id-delta-profile.md) | Every id-ordered site in player_update with the counter shift native needs (pool E flat, pool B creation-order steps), per-construct counter costs, and a profiler for any scratch |
| [pu-residual-map.md](pu-residual-map.md) | player_update residual map by mechanism and verdict, the small-frame packer rule, twelve source respellings that rebuild the native 4-byte frame, and what the alias budget would unlock |
