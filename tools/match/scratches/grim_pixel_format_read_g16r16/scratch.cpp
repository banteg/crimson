class grim_pixel_format_g16r16_t {
public:
    virtual ~grim_pixel_format_g16r16_t();
...
void grim_pixel_format_g16r16_t::read_pixel(
    unsigned int row,
    unsigned int depth,
    float *pixels)
{
    ...
    while (source < end) {
        pixels[0] = (float)(*source & 0xffff) * scale;
        pixels[1] = (float)(*source++ >> 16) * scale;
        pixels[2] = 1.0f;
        pixels[3] = 1.0f;
        pixels += 4;
    }
