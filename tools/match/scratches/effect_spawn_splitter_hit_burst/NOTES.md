...
Three source-shape details are decisive. The loop is a plain
`for (i = 0; i < count; i++)` with `(int)radius` written inside the body. Loop
inversion supplies the entry test, the invariant conversion moves to the
preheader, and the exit test becomes native's `dec`/`jne` countdown. A
`count > 0` guard around the loop re-tests the count, because the inverted test
is `0 < count` and folds only against the same ordered comparison
([plain-float-sources.md](../../c2/compiler/plain-float-sources.md)). Scaling `angle` in two compound-assignment
