class grim_pixel_format_a2w10v10u10_t {
public:
    virtual ~grim_pixel_format_a2w10v10u10_t();
...
void grim_pixel_format_a2w10v10u10_t::read_pixel(
    unsigned int row,
    unsigned int depth,
    float *pixels)
{
    ...
    const float signed_scale = 1.0f / 512.0f;
    while (source < end) {
        short u = (short)*source;
        short v = (short)(*source >> 10);
        short w = (short)(*source >> 20);
        u <<= 6;
        u >>= 6;
        v <<= 6;
        v >>= 6;
        ...
        pixels[2] = (float)w * signed_scale;
        pixels[3] = (*source++ >> 30) * (1.0f / 3.0f);
        pixels += 4;
    }
