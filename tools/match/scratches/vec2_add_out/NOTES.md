...
As with `vec2_sub`, the caller-emitted decorated symbol proves a
`controls_vec2_t` member with `__thiscall`, two raw `float *` ABI parameters,
and a `float *` return. The source recovers the pointed-to pairs as vector
aggregates without changing that evidence-backed signature or the exact code.
