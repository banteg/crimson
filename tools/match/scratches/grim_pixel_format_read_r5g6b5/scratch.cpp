class grim_pixel_format_r5g6b5_t {
public:
    virtual ~grim_pixel_format_r5g6b5_t();
...
void grim_pixel_format_r5g6b5_t::read_pixel(
    unsigned int row, unsigned int depth, float *pixels)
{
    ...
    const float scale_5 = 1.0f / 31.0f;
    while (source < end) {
        pixels[0] = (float)(*source >> 11) * scale_5;
        pixels[1] = (float)((*source >> 5) & 0x3f) * (1.0f / 63.0f);
        pixels[2] = (float)(*source++ & 0x1f) * scale_5;
        pixels[3] = 1.0f;
        pixels += 4;
    }
