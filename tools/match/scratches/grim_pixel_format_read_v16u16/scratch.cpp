class grim_pixel_format_v16u16_t {
public:
    virtual ~grim_pixel_format_v16u16_t();
...
void grim_pixel_format_v16u16_t::read_pixel(
    unsigned int row,
    unsigned int depth,
    float *pixels)
{
    short *source = (short *)(pixel_buffer + row_offset * row + depth_offset * depth);
    short *end = source + width * 2;
    const float scale = 1.0f / 32768.0f;
    while (source < end) {
        pixels[0] = (float)source[0] * scale;
