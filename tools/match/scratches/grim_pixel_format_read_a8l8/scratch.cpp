class grim_pixel_format_a8l8_t {
public:
    virtual ~grim_pixel_format_a8l8_t();
...
void grim_pixel_format_a8l8_t::read_pixel(
    unsigned int row,
    unsigned int depth,
    float *pixels)
{
    ...
    while (source < end) {
        float luminance = (float)(*source & 0xff) * scale;
        pixels[0] = pixels[1] = pixels[2] = luminance;
        pixels[3] = (float)(*source >> 8) * scale;
        ++source;
        pixels += 4;
    }
