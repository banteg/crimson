...
extern "C" void typo_target_name_assign_random(int creature_id)
{
    ...
    while (true) {
        if ((int)highscore_active_record.score_xp > 120) {
            if (crt_rand() % 100 < 10) {
                strcpy(
                    typo_target_name_table[creature_id],
...
validate_name:
        if (typo_target_name_is_unique(
                typo_target_name_table[creature_id], creature_id)) {
            if (strlen(typo_target_name_table[creature_id]) <= 15) {
                return;
            }
            if (attempts++ >= 100) {
                return;
