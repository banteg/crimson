class grim_pixel_format_a8r8g8b8_t {
public:
    virtual ~grim_pixel_format_a8r8g8b8_t();
...
protected:
    ...
    unsigned int depth_first, depth_last;
...
void grim_pixel_format_a8r8g8b8_t::read_pixel(
    unsigned int row, unsigned int depth, float *pixels)
{
    ...
    while (source < end) {
        pixels[0] = (float)((*source >> 16) & 0xff) * scale;
        pixels[1] = (float)((*source >> 8) & 0xff) * scale;
        pixels[2] = (float)(*source & 0xff) * scale;
        pixels[3] = (float)(*source++ >> 24) * scale;
        pixels += 4;
    }
