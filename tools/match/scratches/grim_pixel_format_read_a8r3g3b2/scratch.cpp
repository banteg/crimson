class grim_pixel_format_a8r3g3b2_t {
public:
    virtual ~grim_pixel_format_a8r3g3b2_t();
...
void grim_pixel_format_a8r3g3b2_t::read_pixel(
    unsigned int row, unsigned int depth, float *pixels)
{
    ...
    while (source < end) {
        pixels[0] = (float)((*source >> 5) & 7) * scale;
        pixels[1] = (float)((*source >> 2) & 7) * scale;
        pixels[2] = (float)(*source & 3) * (1.0f / 3.0f);
