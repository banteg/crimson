class grim_pixel_format_a4r4g4b4_t {
public:
    virtual ~grim_pixel_format_a4r4g4b4_t();
...
void grim_pixel_format_a4r4g4b4_t::read_pixel(
    unsigned int row, unsigned int depth, float *pixels)
{
    ...
    while (source < end) {
        pixels[0] = (float)((*source >> 8) & 0x0f) * scale;
        pixels[1] = (float)((*source >> 4) & 0x0f) * scale;
        pixels[2] = (float)(*source & 0x0f) * scale;
        pixels[3] = (float)(*source++ >> 12) * scale;
        pixels += 4;
    }
