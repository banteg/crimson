...
extern "C" void demo_setup_variant_2(void)
{
    ...
    for (; y < 848; ++row, y += 60) {
        ...
        creature_spawn_template(SPAWN_ID_ZOMBIE_RANDOM_41, &pos_0.x, -100.0f);

        vec2f_t pos_1;
        pos_1.x = (float)((side + 2) * 64);
        pos_1.y = row_y;
        creature_spawn_template(SPAWN_ID_ZOMBIE_RANDOM_41, &pos_1.x, -100.0f);

        vec2f_t pos_2;
        pos_2.x = (float)(side * 64 - 64);
        pos_2.y = row_y;
        creature_spawn_template(SPAWN_ID_ZOMBIE_RANDOM_41, &pos_2.x, -100.0f);

        vec2f_t pos_3;
        pos_3.x = (float)((side + 12) * 64);
        pos_3.y = row_y;
        creature_spawn_template(SPAWN_ID_ZOMBIE_RANDOM_41, &pos_3.x, -100.0f);
