class grim_pixel_format_al16_t {
public:
    virtual ~grim_pixel_format_al16_t();
...
void grim_pixel_format_al16_t::read_pixel(
    unsigned int row,
    unsigned int depth,
    float *pixels)
{
    ...
    while (source < end) {
        float luminance = (*source & 0xffff) * scale;
        pixels[0] = pixels[1] = pixels[2] = luminance;
        pixels[3] = (*source++ >> 16) * scale;
        pixels += 4;
    }
