class grim_pixel_format_q8w8v8u8_t {
public:
    virtual ~grim_pixel_format_q8w8v8u8_t();
...
void grim_pixel_format_q8w8v8u8_t::read_pixel(
    unsigned int row,
    unsigned int depth,
    float *pixels)
{
    ...
    while (source < end) {
        ...
        source += 4;
        pixels[2] = (float)source[-2] * scale;
        pixels[3] = (float)source[-1] * scale;
        pixels += 4;
    }
