...
extern "C" int plaguebearer_spread_infection(int creature_id)
{
    ...
    if (creature_pool[other_id].collision_flag && creature_pool[creature_id].health < 150.0f) {
        creature_pool[creature_id].collision_flag = 1;
    }
    if (creature_pool[creature_id].collision_flag && creature_pool[other_id].health < 150.0f) {
