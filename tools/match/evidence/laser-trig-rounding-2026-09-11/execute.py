"""Execute laser-only fixtures with recording external-call contracts."""
...
SPEC = importlib.util.spec_from_file_location("laser_program", ENGINE)
...
def run(program, native, case):
    ...
    assert 0 <= case["player_count"] <= 2
    mu.mem_write(program.address("config_blob") + 20, struct.pack("<I", case["player_count"]))
    ...
    mu.mem_write(program.address("perk_id_sharpshooter"), struct.pack("<I", 2))
    expected_writes = []
    indices = set()
    for row in case["players"]:
        ...
        indices.add(index)
        record = bytearray(0x360)
        struct.pack_into("<2f", record, 0x14, *row["position"])
        struct.pack_into("<f", record, 0x24, row["health"])
        struct.pack_into("<f", record, 0x300, row["heading"])
        struct.pack_into("<i", record, 0xB8 + 4 * 2, row["sharpshooter"])
        address = program.address("player_state_table") + index * 0x360
