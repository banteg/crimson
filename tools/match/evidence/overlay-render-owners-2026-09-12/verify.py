"""Replay overlay owner recovery against native calls and observed stack homes."""
...
BEFORE_SHA = "93a5f988ea28855eb588ce1fa6c6b687ee70c8d8b939b0ac70e592c7bcc98e98"
CURRENT_SHA = "322be4d21391ba19a4ad893e033f2d08238e6393acdafaac2e96b64dc8632172"
...
def main():
    ...
    assert maps["before"]["mismatches"] == 4 and maps["current"]["mismatches"] == 0
    changes = []
    ...
    assert sorted(row["native_offset"] for row in changes) == [2165, 2175, 2522, 2532]
    ...
    assert metrics["current"]["references_ok"] == 338 and metrics["current"]["reference_problems"] == 0
    assert not metrics["current"]["exact"] and not metrics["current"]["body_byte_exact"]
    result = {
        ...
        "restored_stack_accesses": 4, "displaced_stack_accesses": 0,
