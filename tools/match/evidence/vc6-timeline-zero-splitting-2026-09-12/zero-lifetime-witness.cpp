...
struct pointer_pair { int *first; int *last; };
...
spawn_entries:
    ...
    do {
        ...
        if (entry->count > 0) {
            int *template_id; { pointer_pair selected; selected.last = &entry->template_id; selected.first = selected.last; pointer_pair copied; for (unsigned int k = 0; k != sizeof copied; ++k) ((unsigned char *)&copied)[k] = ((unsigned char *)&selected)[k]; template_id = copied.last; }
            int spread;
