"""Bounded particle impact execution through observed native helper bodies.

Creature lookup and sprite spawning execute native code. Damage and randomized
FX helpers can execute native code or remain recording boundaries. Lethal death
handling and the final FX queue enqueue remain recording callbacks. Unknown
control transfers and non-stack writes outside declared observations fail.
"""
...
class Program(m.Program):
    def __init__(self, config, *, integrated=False):
        ...
        self.helpers = {}
        helpers = ["crt_ftol", "vec2_add", "vec2_add_inplace", "fx_spawn_sprite", "creature_find_in_radius"]
        if integrated:
