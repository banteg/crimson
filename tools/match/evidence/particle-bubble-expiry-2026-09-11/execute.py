"""Execute bubble expiry with the real native death-handler prelude.

Derived from the particle-impact runner; inactive target fixtures execute the
native creature_handle_death body through its early return. Unknown transfers
and writes outside the declared pools/globals or stack fail.
"""
...
def run(p, native, case):
    ...
    for name in SCALARS:
        write(name, 0, "B" if name in BYTE_SCALARS else "i")
    ...
    write("survival_recent_death_count", case["history_count"])
    write("survival_reward_fire_seen", case["fire_seen"], "B")
    write("survival_reward_handout_enabled", case["handout_enabled"], "B")
    u.mem_write(p.address("survival_recent_death_pos"), struct.pack("<6f", *case["history_positions"]))
    esp = m.STACK + 0xF000
        ...
        for name, (entry, pcs) in p.helpers.items():
            if a in pcs:
                if a == entry:
                    ...
                    if name == "creature_handle_death":
                        calls.append([name, *struct.unpack("<2I", uc.mem_read(e + 4, 8))])
                    elif name == "vec2_add":
    ...
    allowed = [(p.address(name), p.address(name) + n * size) for name, (n, size) in POOLS.items()] + [
        (
            p.address(n),
            p.address(n) + (1 if n in BYTE_SCALARS else 4),
        )
        for n in SCALARS
    ]
