"""Execute primary impacts and native vector, damage, and randomized FX helpers.

Damage/randomized FX can be recorded boundaries or execute their real bodies.
Final FX enqueue and audio remain recording callbacks. Unknown control transfers
and writes outside observed pools/globals and the stack fail.
"""
...
def run(p, native, case):
    ...
    u.mem_write(p.address("effect_template"), bytes([case.get("template_fill", 0)]) * 60)
    for name in SCALARS:
        write(
            name,
            0,
            "B"
            if name in ("music_playlist_randomized_latch", "fx_queue_random_init_flags", "bonus_spawn_guard")
            else "i",
        )
    write("config_player_count", len(case.get("players", [])))
    write("music_playlist_randomized_latch", 1, "B")
    for item in case.get("primary", []):
        u.mem_write(
            p.address("weapon_table") + 124 * item["type"] + 112,
            struct.pack("<f", case.get("damage_scale", 2)),
        )
    ...
    write("perk_id_ion_gun_master", 51)
    write("perk_id_bloody_mess_quick_learner", 1)
    write("perk_id_poison_bullets", 25)
    write("config_detail_preset", case.get("detail", 0))
    write("perk_id_barrel_greaser", 34)
    ...
    def hook(uc, a, size, data):
        ...
        if a in callbacks:
            ...
            counts = {
                "effect_spawn_ion_hit_core": 3,
                "effect_spawn_plasma_hit_core": 3,
                "effect_spawn_ion_hit_sparks": 2,
                "effect_spawn_explosion_burst": 2,
                "effect_spawn_shrinkifier_hit": 1,
                "effect_spawn_blood_splatter": 3,
                "effect_spawn_splitter_hit_burst": 3,
                "effect_spawn_freeze_shard": 2,
                "projectile_spawn": 4,
            }
            if name in counts:
                ...
                calls.append([name, list(struct.unpack("<2I", raw))])
                length = math.sqrt(x * x + y * y)
                # Deterministic external DLL boundary, not native D3DX execution.
    ...
    allowed = [(p.address(name), p.address(name) + n * size) for name, (n, size) in POOLS.items()] + [
        (
            p.address(n),
            p.address(n)
            + (1 if n in ("music_playlist_randomized_latch", "fx_queue_random_init_flags", "bonus_spawn_guard") else 4),
        )
        for n in SCALARS
    ]
        ...
        "scalars": {
            n: bytes(
                u.mem_read(
                    p.address(n),
                    1
                    if n in ("music_playlist_randomized_latch", "fx_queue_random_init_flags", "bonus_spawn_guard")
                    else 4,
                ),
            )
            for n in SCALARS
