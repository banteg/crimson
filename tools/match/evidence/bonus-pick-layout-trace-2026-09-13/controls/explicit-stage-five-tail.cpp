...
extern "C" bonus_id_t bonus_pick_random_type(void)
{
    ...
    selected:
        ...
        if (config_blob.game_mode == GAME_MODE_QUEST)
        {
            ...
            else if (quest_stage_major == 2 && quest_stage_minor == 10 && bonus_id == BONUS_ID_NUKE)
            {
                continue;
            }
    ...
    common_filters:
        ...
        if ((player_state_table[0].shield_timer > 0.0f ||
             player_state_table[1].shield_timer > 0.0f) &&
