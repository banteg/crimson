...
def run(p, native, case):
    ...
    def hook(uc, a, size, data):
        ...
        for name, (entry, pcs) in p.helpers.items():
            if a in pcs:
                if a == entry:
                    ...
                    if name == "creature_find_nearest":
                        pos, exclude, radius = struct.unpack("<3I", uc.mem_read(e + 4, 12))
                        calls.append([name, list(struct.unpack("<2I", uc.mem_read(pos, 8))), exclude, radius])
                    elif name == "player_find_in_radius":
        ...
        if a in callbacks:
            ...
            if name in ("projectile_spawn", "effect_spawn_ion_hit_core", "effect_spawn_ion_hit_sparks"):
                nargs = {"projectile_spawn": 4, "effect_spawn_ion_hit_core": 3, "effect_spawn_ion_hit_sparks": 2}[name]
                args = list(arg(nargs))
                args[0] = list(struct.unpack("<2I", uc.mem_read(args[0], 8)))
                calls.append([name, *args])
                reply = 1 if name == "projectile_spawn" else 0
