"""Observe and reproduce the canonical HUD's VC6 stack-coloring groups."""
...
def reproduce(symbols, order):
    ...
    # The observed /O2 HUD follows the ascending bucket allocation path.
    # Compute offsets from bucket sizes, then check against observed descriptors.
    offset = -sum((g["size"] + 3) & ~3 for g in groups[1:])
    checked = 0
    for number, group in enumerate(groups):
        group.pop("conflicts")
        group["index"] = number
        group["offset"] = 8 if number == 0 else offset
...
def main():
    ...
    result = {
        "schema_version": 1,
        "kind": "vc6-hud-stack-coloring-observer",
        "verified": True,
        "limitations": "Observes this source and compiler only; the greedy model is scoped to the observed HUD allocation path. No original-source identity, semantic equivalence, or additional exact function is claimed.",
