"""Execute primary impacts and native blood, vector, damage, and randomized FX helpers.

Damage/randomized FX can be recorded boundaries or execute their real bodies.
Final effect allocation, FX enqueue, and audio remain recording callbacks. Unknown control transfers
...
class Program(m.Program):
    def __init__(self, config, *, integrated=False):
        ...
        if integrated:
            helpers.extend(("creature_apply_damage", "fx_queue_add_random", "effect_spawn_blood_splatter"))
...
def run(p, native, case):
    ...
    if "template_scale" in case:
        u.mem_write(p.address("effect_template") + 12, struct.pack("<f", case["template_scale"]))
    for name in SCALARS:
        ...
        for name, (entry, pcs) in p.helpers.items():
            if a in pcs:
                if a == entry:
                    ...
                    elif name == "effect_spawn_blood_splatter":
                        pos, angle, age = struct.unpack("<3I", uc.mem_read(e + 4, 12))
                        calls.append([name, list(struct.unpack("<2I", uc.mem_read(pos, 8))), angle, age])
            ...
            if name == "crt_atexit":
                ...
                calls.append(
                    [
                        ...
                        effect_id,
                        list(struct.unpack("<2I", uc.mem_read(pos, 8))),
                        list(struct.unpack("<15I", uc.mem_read(p.address("effect_template"), 60))),
                    ],
                )
