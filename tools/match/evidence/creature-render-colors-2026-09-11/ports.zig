...
fn drawTextureRegionCenteredRotated(
    ...
    for (current_creatures) |creature| {
        ...
        if (difference < distance) {
            ...
            body_width = creature.size;
        }
    }
    std.debug.assert(distance < 8.0);
    const pass = if (additive) "flash" else if (width == body_width) "body" else "shadow";
    std.debug.assert(width == body_width or @abs(width - body_width * 1.07) < 0.001);
    ...
    output.print("{d} {s} {d} {d} {d} {d} {d} {d} {d} {d} {d}\n", .{ case_index, pass, index, kind, frame, width, height, tint.r, tint.g, tint.b, tint.a }) catch unreachable;
}
