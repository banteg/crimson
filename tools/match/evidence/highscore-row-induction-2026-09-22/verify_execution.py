...
def prepare(name):
    ...
    window = (0x774, 0x876, 0x24) if name == "canonical" else (0x77D, 0x88D, 0x24)
    refmap = {}
...
class Runner:
    ...
    def run(self, fixture):
        ...
        m.mem_write(sp + 0x14, struct.pack("<f", 3.25))
        ...
        y = struct.unpack("<f", m.mem_read(sp + 0x14, 4))[0]
...
def main():
    ...
    for name in ("canonical", "flags-split-mask"):
        build(name, ROOT)
    compiled = {name: prepare(name) for name in ("canonical", "flags-split-mask")}
    ...
    code, window, _, _ = compiled["flags-split-mask"]
    rejected = []
    corruptions = [
        ("wrong-prefix-mask", 0x7D3, b"\x05", b"\x01", (1, 4, 1, 1001, 7)),
        ("unsigned-division-shift", 0x809, b"\xfa", b"\xea", (1, 0, 2, 0x80000000, 7)),
        ("ninety-nine-row-limit", 0x87B, struct.pack("<I", BUFFERS + 16400),
    ...
    result = {
        ...
        "input_hashes": {name: sha((Path(__file__).parent / name).read_bytes())
                         for name in ("controls.py", "controls.json")},
    ...
    print("Verified", len(cases), "native/canonical/selected row fixtures", flush=True)
